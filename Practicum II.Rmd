...
![ERD Diagram Design in MySql WorkBench](https://drive.google.com/uc?export=view&id=1XdBDp8crycgzYW_TvOW7AlnM2Wql1fH4)
...
![Getting up AWS for Processing whole dataset](https://drive.google.com/uc?export=view&id=1Uy4rKHn6_nLXr8BDR-mH7cID8YoFbKlI)
![S3 buckets for storing datasets](https://drive.google.com/uc?export=view&id=1ZCBqQDKRSRprLtNa7sUYaiNs6DwuiU-1)
**title.principals**
```{r}
library(tidyr)
title.principals<-read.csv(text = rawToChar(aws.s3::get_object(object ="s3://parganiha.k.imdb/title.principals.tsv/data.tsv")),sep='\t',na = c("\\N"))
...
```{r}
#library(tidyr)
name.basics<-read.csv(text = rawToChar(aws.s3::get_object(object = "s3://parganiha.k.imdb/name.basics.tsv/data.tsv")),sep='\t',na = c("\\N"))
...
**title.akas**
```{r}
title.akas<-read.csv(text = rawToChar(aws.s3::get_object(object = "s3://parganiha.k.imdb/title.akas.tsv/data.tsv")),sep='\t',na = c("\\N"))
...
**title.basics**
```{r}
title.basics<-read.csv(text = rawToChar(aws.s3::get_object(object=  "s3://parganiha.k.imdb/title.basics.tsv/data.tsv")),sep='\t',na = c("\\N"))
...
**title.crew**
```{r}
title.crew<-read.csv(text = rawToChar(aws.s3::get_object(object = "s3://parganiha.k.imdb/title.crew.tsv/data.tsv")),sep='\t',na = c("\\N"))
...
**title.episode**
```{r,echo=FALSE}
title.episode<-read.csv(text = rawToChar(aws.s3::get_object(object = "s3://parganiha.k.imdb/title.episode.tsv/data.tsv")),sep='\t',na = c("\\N"))
title.episode<- title.episode %>% drop_na()
head(title.episode)
...
title.ratings<-read.csv(text = rawToChar(aws.s3::get_object(object="s3://parganiha.k.imdb/title.ratings.tsv/data.tsv")),sep='\t',na = c("\\N"))
...
library(dplyr)
```
**3. Create and then run CREATE TABLE statements to build the schema. These statements must run from within your notebook and not from a separate script. Ensure proper referential integrity.'**
**Setting up Database**
